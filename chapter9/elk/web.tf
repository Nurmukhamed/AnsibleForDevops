resource "libvirt_volume" "disk_web" {
  name           = "disk-web"
...
data "template_file" "user_data_web" {
  ...
  vars = {
    server_hostname = "web.test"
  }
}

resource "libvirt_cloudinit_disk" "commoninit-web" {
  name           = "commoninit-web.iso"
...
resource "libvirt_domain" "web" {
  ...
  cloudinit = libvirt_cloudinit_disk.commoninit-web.id
  ...
  disk {
    volume_id = libvirt_volume.disk_web.id
  }
