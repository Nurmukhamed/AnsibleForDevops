...
data "template_file" "user_data_ubuntu" {
  ...
  vars = {
    server_hostname = "ubuntu.test"
  }
}

resource "libvirt_cloudinit_disk" "commoninit-ubuntu" {
...
resource "libvirt_domain" "ubuntu" {
  ...
  cloudinit = libvirt_cloudinit_disk.commoninit-ubuntu.id
  ...
  disk {
    volume_id = libvirt_volume.disk_ubuntu.id
  }
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "sudo mkdir -p /vagrant/provisioning",
